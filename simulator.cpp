...
void cellassociation()
{
	...
	if (subcarrier[bs_id] <= SUBCARRIER)
			{
                      	   subcarrier[bs_id]=subcarrier[bs_id] + 12;
...
void run_simulation()
 {
  ...
  //   cuser=0;
							 ...
							 ho_thr=base[j].hothreshold();
										...
										/*Estimating data rate from target network*/
 										      ...
 										      else if (hard_handoff)  /*hard handover is going to occur*/
 											{
  											    ...
  											    ob[i].timer=ob[i].timer+20;  // to avoid ping pong effect
   ...
   float mean, range, confidence, CI, sample, dev_temp, std_dev;
   dev_temp=0;
   mean=temp_outage/(temp_timer*CBR);
  

   for (int k=0; k < NO_OF_UE; k++)
         {
                
                sample=ob[k].outage/(ob[k].timer*CBR);
               
                dev_temp=dev_temp+(mean-sample)*(mean-sample);
		
	 }

  std_dev=sqrt(dev_temp/NO_OF_UE);
  range=RANGE*(std_dev/sqrt(NO_OF_UE));
 ...
 //  cout << "System throughput (Mbps): " << (temp_throughput/temp_timer) << "\n";
  ...
  cout << "Confidence interval: " << mean - range << "," << mean + range << "\n";
  cout << "Margin of error: " << range << "\n";
 }
